...
export function BrandForm({
  ...
  const [countryOrigin, setCountryOrigin] = useState(initial?.country_origin ?? "");
  ...
  function buildFormData(): FormData {
    ...
    if (countryOrigin) formData.set("country_origin", countryOrigin);
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e, false)} className="flex flex-col gap-6">
      <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-6">
        <h2 className="mb-5 text-sm font-semibold text-white/70">Détails de la marque</h2>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-1.5 block text-sm text-white/70">Pays d&apos;origine</label>
            <input
              value={countryOrigin}
          ...
          <div className="sm:col-span-2">
            <label className="mb-1.5 block text-sm text-white/70">Logo</label>
            <label
              onDragOver={(e) => {
                e.preventDefault();
                setDragOver(true);
