...
import { HomepageSectionForm } from "@/components/homepage-section-form";
...
export default function CreateHomepageSectionPage() {
  ...
  async function handleSubmit(values: Record<string, unknown>, addAnother: boolean) {
    ...
    setSubmitting(true);
    try {
      await apiFetch("/admin/homepage-sections", { method: "POST", body: JSON.stringify(values) });
      if (addAnother) {
        setFormKey((k) => k + 1);
      } else {
  ...
  return (
    <div className="-m-8 min-h-screen bg-[#0b0d12] p-8 text-white">
      ...
      <h1 className="mb-6 text-2xl font-bold">Créer une section</h1>
