...
import { exportOrdersSummaryToExcel, exportOrdersSummaryToPdf, OrdersSummaryRow } from "@/lib/reportExport";
...
export default function RapportCommandesPage() {
  ...
  const [rows, setRows] = useState<OrdersSummaryRow[] | null>(null);
  const [meta, setMeta] = useState<Paginated<OrdersSummaryRow>["meta"] | null>(null);
  ...
  function load() {
    apiFetch<Paginated<OrdersSummaryRow>>(`/admin/reports/orders-summary?${buildQuery().toString()}`)
      .then((res) => {
        setRows(res.data);
        setMeta(res.meta);
        setError(null);
  ...
  async function fetchAllRows(): Promise<OrdersSummaryRow[]> {
    const query = buildQuery(1000);
    query.set("page", "1");
    const res = await apiFetch<Paginated<OrdersSummaryRow>>(`/admin/reports/orders-summary?${query.toString()}`);
    return res.data;
  }
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      exportOrdersSummaryToExcel(all, `rapport-commandes-${from}-${to}`);
    } catch {
  ...
  async function handleExportPdf() {
    ...
    try {
      const all = await fetchAllRows();
      exportOrdersSummaryToPdf(all, `rapport-commandes-${from}-${to}`, "Rapport récapitulatif des commandes");
    } catch {
  ...
  return (
    <div className="-m-8 min-h-screen bg-[#0b0d12] p-8 text-white">
      <div className="mb-4 flex items-center gap-1.5 text-xs text-white/40">
        ...
        <span>Rapport récapitulatif des commandes</span>
      </div>

      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Rapport récapitulatif des commandes</h1>
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.03]">
        ...
        {rows?.length === 0 && (
          <p className="px-5 py-10 text-center text-sm text-white/30">Aucune commande pour cette période.</p>
        )}
