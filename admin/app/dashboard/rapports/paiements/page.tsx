...
import { exportPaymentsReportToExcel, exportPaymentsReportToPdf, PaymentReportRow } from "@/lib/reportExport";
...
export default function RapportPaiementsPage() {
  ...
  const [rows, setRows] = useState<PaymentReportRow[] | null>(null);
  const [meta, setMeta] = useState<Paginated<PaymentReportRow>["meta"] | null>(null);
  ...
  function load() {
    apiFetch<Paginated<PaymentReportRow>>(`/admin/reports/payments?${buildQuery().toString()}`)
      .then((res) => {
        setRows(res.data);
        setMeta(res.meta);
        setError(null);
  ...
  async function fetchAllRows(): Promise<PaymentReportRow[]> {
    const query = buildQuery(1000);
    query.set("page", "1");
    const res = await apiFetch<Paginated<PaymentReportRow>>(`/admin/reports/payments?${query.toString()}`);
    return res.data;
  }
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      exportPaymentsReportToExcel(all, `rapport-paiements-${from}-${to}`);
    } catch {
  ...
  async function handleExportPdf() {
    ...
    try {
      const all = await fetchAllRows();
      exportPaymentsReportToPdf(all, `rapport-paiements-${from}-${to}`, "Rapport des paiements");
    } catch {
  ...
  return (
    <div className="-m-8 min-h-screen bg-[#0b0d12] p-8 text-white">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Rapport des paiements</h1>
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.03]">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {rows?.map((row, idx) => (
              <tr key={`${row.order_reference}-${idx}`} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="px-5 py-3 text-white/60">{row.payment_method}</td>
                ...
                <td className="px-5 py-3 text-white">{formatXAF(row.amount)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {rows?.length === 0 && (
          <p className="px-5 py-10 text-center text-sm text-white/30">Aucun paiement pour cette période.</p>
        )}
