...
import { exportPendingPaymentsToExcel, exportPendingPaymentsToPdf, PendingPaymentRow } from "@/lib/reportExport";

const PAYMENT_STATUSES = ["en_attente", "échoué"];
...
export default function RapportPaiementsAttentePage() {
  ...
  const [rows, setRows] = useState<PendingPaymentRow[] | null>(null);
  const [meta, setMeta] = useState<Paginated<PendingPaymentRow>["meta"] | null>(null);
  ...
  function load() {
    apiFetch<Paginated<PendingPaymentRow>>(`/admin/reports/receivables?${buildQuery().toString()}`)
      .then((res) => {
        setRows(res.data);
        setMeta(res.meta);
        setError(null);
  ...
  async function fetchAllRows(): Promise<PendingPaymentRow[]> {
    const query = buildQuery(1000);
    query.set("page", "1");
    const res = await apiFetch<Paginated<PendingPaymentRow>>(`/admin/reports/receivables?${query.toString()}`);
    return res.data;
  }
  ...
  async function handleExportExcel() {
    ...
    try {
      ...
      exportPendingPaymentsToExcel(all, `paiements-attente-${from}-${to}`);
    } catch {
  ...
  async function handleExportPdf() {
    ...
    try {
      const all = await fetchAllRows();
      exportPendingPaymentsToPdf(all, `paiements-attente-${from}-${to}`, "Rapport des paiements en attente");
    } catch {
  ...
  return (
    <div className="-m-8 min-h-screen bg-[#0b0d12] p-8 text-white">
      <div className="mb-4 flex items-center gap-1.5 text-xs text-white/40">
        ...
        <span>Rapport des paiements en attente</span>
      </div>

      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Rapport des paiements en attente</h1>
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.03]">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-white/5 text-xs uppercase tracking-wide text-white/30">
              ...
              <th className="px-5 py-3 font-medium">État du paiement</th>
          ...
          <tbody>
            {rows?.map((row) => (
              <tr key={row.order_reference} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                      PAYMENT_STYLES[row.payment_status] ?? "bg-white/5 text-white/60"
                    }`}
                  >
                    {row.payment_status}
                  </span>
                ...
                <td className="px-5 py-3 font-medium text-red-400">{formatXAF(row.amount_due)}</td>
        ...
        {rows?.length === 0 && (
          <p className="px-5 py-10 text-center text-sm text-white/30">Aucun paiement en attente pour cette période.</p>
        )}
