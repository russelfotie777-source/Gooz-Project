...
class ReportController extends Controller
{
    ...
    public function pendingPayments(Request $request)
    {
        ...
        $paginator = Order::query()
            ->with(['user', 'payment'])
            ->whereBetween('created_at', [$from, $to])
            ->whereHas('payment', fn ($p) => $p->whereIn('payment_status', ['en_attente', 'échoué']))
            ->when($status, fn ($q) => $q->whereHas('payment', fn ($p) => $p->where('payment_status', $status)))
            ->when($paymentMethod, fn ($q) => $q->whereHas('payment', fn ($p) => $p->where('payment_method', $paymentMethod)))
            ->when($search, fn ($q) => $q->where(function ($query) use ($search) {
                $query->where('order_reference', 'like', "%{$search}%")
                    ->orWhereHas('user', fn ($u) => $u->where('name', 'like', "%{$search}%")
                        ->orWhere('email', 'like', "%{$search}%")
